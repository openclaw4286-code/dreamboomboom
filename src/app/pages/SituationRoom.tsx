...
export default function SituationRoom() {
  ...
  const maxRow =
    displayAgents.length > 0
      ? Math.max(...displayAgents.map((a) => a.position.row))
      : 0;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                ...
                <span>
                  즐겨찾기{" "}
                  {favoriteAgents.length > 0 && `(${favoriteAgents.length})`}
                </span>
              </button>
              <div className="hidden sm:block">
                <p className="text-slate-400 text-xs mb-0.5">현재 시각</p>
              ...
              <div>
                <p className="text-slate-400 text-xs mb-0.5">대기중인 Agent</p>
                <p className="text-white text-sm font-semibold">
                  {agents.filter((a) => !a.isWorking).length}명
                </p>
              </div>
            </div>
          </div>
              ...
              {grid.map((row, rowIndex) => (
                ...
                >
                  {row.map((agent, colIndex) => {
                    if (!agent) return <div key={colIndex} className="hidden sm:block lg:block" />;
                    return (
                      <motion.button
                        key={agent.id}
                        onClick={() => handleAgentClick(agent)}
                        disabled={agent.isWorking}
                        className={`group relative bg-slate-800/50 backdrop-blur-sm border-2 rounded-xl p-4 md:p-6 transition-all duration-300 w-full ${
                          agent.isWorking
                        ...
                        whileHover={agent.isWorking ? {} : { scale: 1.03 }}
                        whileTap={agent.isWorking ? {} : { scale: 0.98 }}
                      >
                        {/* Favorite Star */}
                        <button
                          onClick={(e) => toggleFavorite(agent.id, e)}
                          className="absolute top-3 right-3 z-10 p-1 rounded-full hover:bg-slate-700 transition-all"
                        >
                          <Star
                        ...
                        <div className="flex flex-col items-center text-center pt-2">
                          ...
                          <p className="text-slate-400 text-sm mb-2 md:mb-3">
                            {agent.role}
                          </p>
